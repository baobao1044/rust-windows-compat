//! COM vtable layer for D3D11/DXGI (workstream M7a).
//!
//! This crate wraps the existing Rust-native D3D11/DXGI-over-Vulkan translation
//! (`nigg-d3d11`, `nigg-dxgi`) in COM objects a Windows PE can drive through real
//! vtables.
//!
//! # The problem this solves
//!
//! A PE that links `d3d11.dll`/`dxgi.dll` calls `D3D11CreateDeviceAndSwapChain`
//! through the Import Address Table (an import → IAT → Win64->SysV thunk → our
//! Rust impl). That call hands back COM interface pointers (`ID3D11Device*`,
//! `IDXGISwapChain*`, `ID3D11DeviceContext*`). The PE then calls methods through
//! the object's vtable:
//!
//! ```text
...
//! loader passes a thunk-creation callback into [`ComVtables::install`]; this
//! crate uses it to allocate the per-slot thunks at load time, before the arena
//! is sealed (flipped to `PROT_EXEC`).
//!
//! # COM object layout
//!
...
//! # Vtable layout (M7a)
//!
//! Each interface vtable is a `#[repr(C)]` struct of function pointers. The
//! slot order is the order declared in the corresponding `*Vtbl` struct below;
//! it is a **minimal, documented** layout (IUnknown + only the methods a
//! clear-colour sample needs), not the full Windows vtable. The hand-written
//! D3D11 sample fixture calls methods by these exact byte offsets, so the
//! layout is internally consistent. Full Windows-vtable-order fidelity (so
//! real D3D11 PEs index the right slots) is deferred to a later milestone.
//!
//! `#![deny(unsafe_op_in_unsafe_fn)]` is enforced; every `unsafe` block has a
//! SAFETY comment.

#![deny(unsafe_op_in_unsafe_fn)]
// The COM vtable structs, inner wrappers, and no-op stubs model the full M7a
// method surface even when only a subset is exercised by the clear-colour
// sample; keeping them avoids surprising `dead_code` churn as more methods
// light up in later milestones.
#![allow(dead_code)]
...
pub const DXGI_ERROR_INVALID_CALL: i32 = 0x887A_0001u32 as i32;
...
/// `IUnknown::QueryInterface` — not implemented for M7a (returns
/// `E_NOINTERFACE` and nulls the out-pointer). The clear-colour sample never
/// calls it.
...
pub(crate) fn vtables() -> &'static ComVtables {
    VTABLES
        .get()
        .expect("nigg-d3d11-com: vtables not installed; call install_vtables at load time")
}
...
/// One import-level D3D11/DXGI export the PE loader registers in its `ImplTable`:
...
/// The D3D11/DXGI import-level exports (`D3D11CreateDeviceAndSwapChain`,
/// `CreateDXGIFactory`, ...). The PE loader iterates these and registers a
/// thunk for each, exactly like its kernel32/user32 exports.
